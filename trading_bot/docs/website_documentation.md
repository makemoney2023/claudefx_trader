...
- **Claude Opus 5 everywhere**: chart analysis, trade judge, position/pending re-evals, trade reviews, weekly insights, and sizing all run on Opus 5 with adaptive thinking (low effort for analysis with a 16k budget + thinking-disabled forced-tool retry at 32k on any max_tokens stop; medium for judge/reviews; low for sizing/re-evals). Pre-Claude viability skips the LLM when the only-viable direction conflicts with extreme premium/discount (avoids paying for setups the limit-zone gate will hard-block). Tool JSON is capped (2–4 sentence reasoning, max 3 OB/FVG/liq/warning labels).
