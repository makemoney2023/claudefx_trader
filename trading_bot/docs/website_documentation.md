...
- **Claude Opus 5 everywhere**: chart analysis, trade judge, position/pending re-evals, trade reviews, weekly insights, and sizing all run on Opus 5 with adaptive thinking (low effort for analysis with a 16k budget + thinking-disabled forced-tool retry at 32k on any max_tokens stop; medium for judge/reviews; low for sizing/re-evals). Limit-zone validation uses the **entry** retrace (not spot), so anticipatory sell_limits into premium are allowed while price is still in discount. Tool JSON is capped (2–4 sentence reasoning, max 3 OB/FVG/liq/warning labels).
