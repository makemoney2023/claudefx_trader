...
- **Claude Opus 5 everywhere**: chart analysis, trade judge, position/pending re-evals, trade reviews, weekly insights, and sizing all run on Opus 5 with adaptive thinking (low effort for analysis with a 16k budget + thinking-disabled forced-tool retry at the same 16k budget on any max_tokens stop; medium for judge/reviews; low for sizing/re-evals)
